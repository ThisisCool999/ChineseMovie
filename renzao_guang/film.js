...
const Aud=(function(){
  ...
  function init(){
    ...
    try{
      ...
      const tone=actx.createBiquadFilter();tone.type="lowpass";tone.frequency.value=2600;tone.Q.value=.5;
      master.connect(tone);tone.connect(comp);comp.connect(actx.destination);
      // drone
  ...
  function on(){enabled=false;}   // generative ambience retired — real voice + 6-cue score carry the film now
  ...
  function setMood(act){
    ...
    droneG.gain.linearRampToValueAtTime(0,tnow+2.5);
...
const Film=(function(){
  ...
  function frame(now){
    ...
    if(window.Trax)Trax.update(t,prevT,playing);
    // ui
  ...
  /* ---------- transport ---------- */
  function play(){if(t>=DUR)t=0;playing=true;Fxs.start();if(window.Trax)Trax.resume();flashUI();}
  function pause(){playing=false;PASpk.stop();if(window.Trax)Trax.pause();flashUI();}
  ...
  function seek(time){
    ...
    PASpk.stop(); updatePA(t,t,false); if(window.Trax)Trax.seek(t);
  }
  ...
  function toggleSound(){
    ...
    if(window.Trax)Trax.enable(soundOn);
  }
  ...
  function boot(){
    dom={
      poster:$("#poster"),start:$("#startBtn"),startScore:$("#startBtnScore"),cinema:$("#cinema"),root:$("#cinema"),
    ...
    // start gate
    function beginShow(withVoice){
      if(started)return;started=true;
      if(window.Trax)Trax.setVoice(withVoice);
      dom.poster.classList.add("gone");
      dom.cinema.classList.add("on");dom.cinema.setAttribute("aria-hidden","false");
      if(!soundOn)toggleSound();   // start click is the audio gesture
      setTimeout(()=>{seek(0);play();},700);
    }
    dom.start.addEventListener("click",()=>beginShow(true));
    if(dom.startScore)dom.startScore.addEventListener("click",()=>beginShow(false));
    // controls
  ...
  layer.innerHTML=
    '<div style="position:absolute;inset:0">'+svg("0 0 1600 900",
      ...
      '<g id="qfb" transform="translate(905,470)">'+
        ...
        '<path d="M-40 76 C-56 112 -54 174 -48 204 L-35 204 C-39 170 -40 118 -31 80 Z" fill="#aab6ba"/><ellipse cx="-44" cy="208" rx="8" ry="10" fill="#cbb097"/>'+
...
SC.montage2={build(layer){
  function rays(stopY,color,op){let s='';for(let i=0;i<9;i++){const x=320+i*120;s+='<line x1="'+(x-(x-800)*0.18).toFixed(0)+'" y1="60" x2="'+x+'" y2="'+stopY+'" stroke="'+color+'" stroke-width="5" opacity="'+op+'"/>';}return s;}
  function arcY(x){let best=420,bd=1e9;for(let t=0;t<=1;t+=0.008){const m=1-t,bx=m*m*m*120+3*m*m*t*500+3*m*t*t*1100+t*t*t*1480;if(Math.abs(bx-x)<bd){bd=Math.abs(bx-x);best=m*m*m*420+3*m*m*t*336+3*m*t*t*336+t*t*t*420;}}return best;}
  let raysA='';for(let i=0;i<9;i++){const x=320+i*120,sy=arcY(x)-6;raysA+='<line x1="'+(x-(x-800)*0.18).toFixed(0)+'" y1="70" x2="'+x+'" y2="'+sy.toFixed(0)+'" stroke="#ffcf7a" stroke-width="5" opacity=".5"/>';}
  const A=svg("0 0 1600 900",
    '<defs><linearGradient id="m2skA" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#0c1a30"/><stop offset="1" stop-color="#22426a"/></linearGradient></defs>'+
    '<rect width="1600" height="900" fill="url(#m2skA)"/>'+
    '<circle cx="800" cy="86" r="124" fill="#ffd27a" opacity=".3"/><circle cx="800" cy="86" r="72" fill="#fff2cc"/>'+
    raysA+
    '<path d="M120 420 C500 336 1100 336 1480 420" stroke="#5fd6e6" stroke-width="22" fill="none"/>'+
    '<path d="M120 420 C500 336 1100 336 1480 420" stroke="#cdf3fb" stroke-width="6" fill="none" opacity=".7"/>'+
    '<path d="M-100 900 C400 640 1200 640 1700 900 Z" fill="#15324a"/>'+
    '<g fill="#234a62"><rect x="610" y="600" width="40" height="120"/><rect x="710" y="576" width="34" height="144"/><rect x="900" y="596" width="44" height="124"/><rect x="986" y="572" width="34" height="148"/></g>'+
    '<text x="800" y="180" text-anchor="middle" font-family="sans-serif" font-size="52" font-weight="700" fill="#cdf3fb">臭氧层 = 天空的「盾」</text>'+
    '<text x="800" y="232" text-anchor="middle" font-family="sans-serif" font-size="32" fill="#8fbfd0">挡住阳光里的毒</text>');
  const B=svg("0 0 1600 900",
    '<defs><linearGradient id="m2skB" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#1a1228"/><stop offset="1" stop-color="#3a1c2a"/></linearGradient></defs>'+
    '<rect width="1600" height="900" fill="url(#m2skB)"/>'+
    '<circle cx="800" cy="86" r="156" fill="#ff7a4a" opacity=".4"/><circle cx="800" cy="86" r="78" fill="#ffd9a0"/>'+
    rays(900,'#ff5a3a',.55)+
    '<path d="M120 420 C500 336 1100 336 1480 420" stroke="#7a2a1a" stroke-width="10" fill="none" opacity=".7" stroke-dasharray="28 30"/>'+
    '<path d="M-100 900 C400 660 1200 660 1700 900 Z" fill="#3a1410"/>'+
    '<text x="800" y="180" text-anchor="middle" font-family="sans-serif" font-size="52" font-weight="700" fill="#ff9a7a">盾，没了</text>'+
    '<text x="800" y="232" text-anchor="middle" font-family="sans-serif" font-size="36" fill="#ffb89a">阳光，第一次变成了「毒」</text>');
  const C=svg("0 0 1600 900",
    '<rect width="1600" height="900" fill="#15110f"/>'+
    '<g transform="translate(420,440)"><ellipse cx="0" cy="-70" rx="56" ry="62" fill="#3a4750"/><rect x="-66" y="-20" width="132" height="190" rx="36" fill="#3a4750"/></g>'+
    '<g fill="#5fd6e6"><path d="M300 300 l26 -11 l26 11 l0 32 q0 28 -26 40 q-26 -12 -26 -40 z"/><path d="M300 470 l26 -11 l26 11 l0 32 q0 28 -26 40 q-26 -12 -26 -40 z" opacity=".45"/><path d="M520 300 l26 -11 l26 11 l0 32 q0 28 -26 40 q-26 -12 -26 -40 z" opacity=".7"/></g>'+
    '<polygon points="900,0 1060,0 720,560 560,560" fill="#ff5a3a" opacity=".2"/>'+
    '<text x="900" y="360" font-family="monospace" font-size="92" font-weight="700" fill="#ff6a5a">免疫 → 0</text>'+
    '<g transform="translate(1080,520)"><circle cx="0" cy="0" r="42" fill="#e2c6aa"/><ellipse cx="0" cy="78" rx="46" ry="34" fill="#cdd6da"/></g>'+
    '<text x="800" y="180" text-anchor="middle" font-family="sans-serif" font-size="50" font-weight="700" fill="#ffd0c4">免疫归零 · 像刚出生的婴儿</text>'+
    '<text x="800" y="232" text-anchor="middle" font-family="sans-serif" font-size="30" fill="#c79a8c">这辈子的抵抗力，全部清空</text>');
  const D=svg("0 0 1600 900",
    '<rect width="1600" height="900" fill="#12161a"/>'+
    '<g transform="translate(380,440)"><ellipse cx="0" cy="-70" rx="60" ry="66" fill="#46535c"/><rect x="-70" y="-16" width="140" height="190" rx="38" fill="#46535c"/></g>'+
    '<g fill="#9fd0e6"><circle cx="470" cy="350" r="10"/><circle cx="512" cy="372" r="7"/><circle cx="500" cy="326" r="6"/><circle cx="548" cy="356" r="8"/></g>'+
    '<text x="470" y="300" font-family="sans-serif" font-size="40" fill="#9fd0e6">普通感冒</text>'+
    '<path d="M640 480 L880 480 L920 380 L960 600 L1000 480 L1060 480 L1480 480" fill="none" stroke="#ff5a4a" stroke-width="6"/>'+
    '<circle cx="1480" cy="480" r="9" fill="#ff5a4a"/>'+
    '<text x="800" y="180" text-anchor="middle" font-family="sans-serif" font-size="50" font-weight="700" fill="#ff9a7a">一场普通感冒，就能要命</text>');
  const E=svg("0 0 1600 900",
    '<defs><radialGradient id="m2glowE" cx="40%" cy="34%" r="70%"><stop offset="0" stop-color="#ff7a5a" stop-opacity=".75"/><stop offset="100%" stop-color="#ff7a5a" stop-opacity="0"/></radialGradient></defs>'+
    '<rect width="1600" height="900" fill="#0e1418"/>'+
    '<circle cx="440" cy="470" r="200" fill="#3a1410"/><circle cx="440" cy="470" r="206" fill="url(#m2glowE)"/>'+
    '<g stroke="#5a2018" stroke-width="2" opacity=".55" fill="none"><ellipse cx="440" cy="470" rx="200" ry="68"/><ellipse cx="440" cy="470" rx="200" ry="132"/><line x1="240" y1="470" x2="640" y2="470"/><line x1="440" y1="270" x2="440" y2="670"/></g>'+
    '<g fill="none" stroke-width="3" opacity=".6"><path d="M780 660 C940 640 1140 614 1470 600" stroke="#4fd2e0"/><path d="M780 678 C1000 660 1220 620 1470 500" stroke="#7ad27a"/></g>'+
    '<path d="M780 668 C1010 666 1210 636 1320 566 C1400 510 1430 320 1450 200" fill="none" stroke="#ff6a5a" stroke-width="5"/>'+
    '<circle cx="1450" cy="200" r="9" fill="#ff6a5a"/>'+
    '<text x="800" y="180" text-anchor="middle" font-family="sans-serif" font-size="46" font-weight="700" fill="#ffd0c4">全球 · 所有季节 · 一起逆转</text>'+
    '<text x="1160" y="430" text-anchor="middle" font-family="sans-serif" font-size="30" fill="#c79a8c">模型全部失效</text>');
  layer.innerHTML=
    '<div id="mA" style="position:absolute;inset:0">'+A+'</div>'+
    '<div id="mB" style="position:absolute;inset:0;opacity:0">'+B+'</div>'+
    '<div id="mC" style="position:absolute;inset:0;opacity:0">'+C+'</div>'+
    '<div id="mD" style="position:absolute;inset:0;opacity:0">'+D+'</div>'+
    '<div id="mE" style="position:absolute;inset:0;opacity:0">'+E+'</div>';
  const P=['#mA','#mB','#mC','#mD','#mE'].map(s=>layer.querySelector(s));
  const BD=[0,.19,.38,.58,.76,1],f=.03;
  return{update(p){
    P.forEach((el,i)=>{const a=BD[i],b=BD[i+1];let o=0;
      if(p>=a-f&&p<=b+f){const up=i===0?1:clamp((p-(a-f))/(2*f),0,1),dn=i===4?1:clamp((b+f-p)/(2*f),0,1);o=Math.min(up,dn);}
      el.style.opacity=o.toFixed(2);el.style.transform='scale('+(1.02+p*0.05).toFixed(3)+')';});
  }};
}};
...
SC.memo={build(layer){
  let body='';for(let i=0;i<3;i++){const y=118+i*40,w=540-(i%3)*90;body+='<rect x="40" y="'+y+'" width="'+w+'" height="12" rx="4" fill="#b8ad98"/>';}
  layer.innerHTML='<div style="position:absolute;inset:0">'+svg("0 0 1600 900",
    ...
    '<g transform="translate(706,300) rotate(-3)">'+
      ...
      '<text x="38" y="48" font-family="sans-serif" font-size="30" fill="#3a342a" font-weight="700">柏林中心医院 · 供体名册</text>'+
      '<g transform="translate(0,30)">'+body+'</g>'+
      '<rect id="hl" x="34" y="250" width="486" height="50" rx="4" fill="#ffd24a" opacity="0"/>'+
      '<text x="46" y="286" font-family="sans-serif" font-size="33" fill="#1c1812" font-weight="700">千分之二：对光毒，天生免疫。</text>'+
      '<ellipse id="ring" cx="112" cy="272" rx="100" ry="32" fill="none" stroke="#c0392b" stroke-width="5" opacity="0"/>'+
      '<rect x="40" y="362" width="640" height="12" rx="4" fill="#b8ad98"/><rect x="40" y="400" width="500" height="12" rx="4" fill="#b8ad98"/><rect x="40" y="438" width="560" height="12" rx="4" fill="#b8ad98"/>'+
    '</g>'+
  "")+'</div>';
...
SC.finalTitle={build(layer){
  let buildings='';for(let i=0;i<22;i++){const x=i*74+8,h=44+((i*61)%150),y=614-h;buildings+='<rect x="'+x+'" y="'+y+'" width="58" height="'+h+'"/>';}
  layer.innerHTML='<div style="position:absolute;inset:0">'+svg("0 0 1600 900",
    '<defs>'+
      '<linearGradient id="ftSky" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#171030"/><stop offset="30%" stop-color="#4e2848"/><stop offset="56%" stop-color="#b14e3c"/><stop offset="80%" stop-color="#ef9a4e"/><stop offset="100%" stop-color="#ffd886"/></linearGradient>'+
      '<radialGradient id="ftSun" cx="50%" cy="50%" r="50%"><stop offset="0" stop-color="#fffaf0"/><stop offset="40%" stop-color="#ffe6a0"/><stop offset="72%" stop-color="#ff9a4a"/><stop offset="100%" stop-color="rgba(255,120,60,0)"/></radialGradient>'+
      '<linearGradient id="ftSea" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#a8502f"/><stop offset="1" stop-color="#2e1422"/></linearGradient>'+
    '</defs>'+
    '<rect width="1600" height="900" fill="url(#ftSky)"/>'+
    '<circle id="ftGlow" cx="800" cy="606" r="440" fill="url(#ftSun)" opacity="0"/>'+
    '<circle id="ftSun2" cx="800" cy="606" r="152" fill="#fff4d4"/>'+
    '<rect y="614" width="1600" height="286" fill="url(#ftSea)"/>'+
    '<polygon id="ftRefl" points="748,614 852,614 904,900 696,900" fill="#ffcf86" opacity=".45"/>'+
    '<g fill="#160c16"><g opacity=".97">'+buildings+'</g>'+
      '<path d="M512 614 A286 286 0 0 1 1088 614 Z" fill="#120810" opacity=".55"/>'+
      '<path d="M512 614 A286 286 0 0 1 1088 614" fill="none" stroke="#3a2230" stroke-width="3" opacity=".7"/>'+
    '</g>'+
    '<rect x="0" y="610" width="1600" height="8" fill="#0e0810"/>'+
  "")+'</div>'+
    '<div id="ftScrim" style="position:absolute;inset:0;background:linear-gradient(180deg,rgba(8,4,12,.42),rgba(8,4,12,0) 28%,rgba(8,4,12,0) 58%,rgba(8,4,12,.62));pointer-events:none"></div>'+
    '<div class="center" style="flex-direction:column;gap:5%;pointer-events:none">'+
      '<div id="poem" class="sub poem" style="opacity:0;max-width:76%;position:static;color:#fdeede">因为相信谎言，比相信这杀人的阳光，曾经温柔过，要容易得多。</div>'+
      '<div id="credit" style="opacity:0;text-align:center;font-family:var(--serif);letter-spacing:.34em;color:#fff3d8;text-shadow:0 2px 36px rgba(0,0,0,.7)"><div style="font-size:3.1vw">人造光</div></div>'+
    '</div>'+
    '<div id="ftDark" style="position:absolute;inset:0;background:#000;opacity:0;pointer-events:none"></div>';
  const glow=layer.querySelector('#ftGlow'),sun=layer.querySelector('#ftSun2'),refl=layer.querySelector('#ftRefl'),poem=layer.querySelector('#poem'),credit=layer.querySelector('#credit'),dark=layer.querySelector('#ftDark');
  return{update(p,t){
    const sink=clamp((p-.05)/.95,0,1)*200, cy=606+sink, setAmt=clamp((p-.5)/.5,0,1);
    if(glow){glow.setAttribute('cy',cy.toFixed(0));glow.setAttribute('opacity',(clamp(p/.3,0,1)*(.8+Math.sin(t*1.1)*.1)*(1-setAmt)).toFixed(2));}
    if(sun)sun.setAttribute('cy',cy.toFixed(0));
    if(refl)refl.setAttribute('opacity',(clamp(p/.3,0,1)*(.4+Math.sin(t*2.2)*.12)*(1-clamp((p-.45)/.45,0,1))).toFixed(2));
    if(poem)poem.style.opacity=(clamp((p-.08)/.24,0,1)*(p>.55?lerp(1,0,(p-.55)/.16):1)).toFixed(2);
    if(credit)credit.style.opacity=(clamp((p-.6)/.16,0,1)*(p>.9?lerp(1,0,(p-.9)/.1):1)).toFixed(2);
    if(dark)dark.style.opacity=clamp((p-.8)/.2,0,1).toFixed(2);
  }};
}};
...
SC.patent={build(layer){
  layer.innerHTML='<div style="position:absolute;inset:0">'+svg("0 0 1600 900",
    '<defs><linearGradient id="ptRoom" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#232c31"/><stop offset="1" stop-color="#10171b"/></linearGradient></defs>'+
    '<rect width="1600" height="900" fill="url(#ptRoom)"/>'+
    '<ellipse cx="800" cy="450" rx="720" ry="430" fill="#dfeefc" opacity=".08"/>'+
    '<g transform="translate(360,118)">'+
      ...
      '<text x="40" y="66" font-family="serif" font-size="40" fill="#2a2620" font-weight="700">专 利 登 记 证</text>'+
      '<line x1="40" y1="86" x2="832" y2="86" stroke="#9a8e74" stroke-width="2"/>'+
      '<text x="40" y="156" font-family="sans-serif" font-size="31" fill="#3a342a">名称：天穹滤光膜 · 光毒过滤穹顶</text>'+
      '<text x="40" y="216" font-family="sans-serif" font-size="31" fill="#3a342a">注册公司：长 夜</text>'+
      '<text x="40" y="300" font-family="sans-serif" font-size="30" fill="#3a342a">登记日期：</text>'+
      '<g id="datebox" transform="translate(292,300)" opacity="0">'+
        '<rect x="0" y="-40" width="476" height="60" rx="6" fill="#fbeaea" stroke="#c0392b" stroke-width="4"/>'+
        '<text x="238" y="4" text-anchor="middle" font-family="sans-serif" font-size="36" fill="#c0392b" font-weight="700">灾难前 · 整整两年</text>'+
      '</g>'+
      '<text x="40" y="398" font-family="monospace" font-size="22" fill="#6a6048">备注：长夜公司，成立于同年。此前，无任何记录。</text>'+
      '<g fill="#b8ad98"><rect x="40" y="446" width="700" height="12" rx="4"/><rect x="40" y="484" width="560" height="12" rx="4"/><rect x="40" y="522" width="650" height="12" rx="4"/></g>'+
      '<g id="stamp" transform="translate(650,556) rotate(-13)" opacity="0">'+
        '<circle r="70" fill="none" stroke="#c0392b" stroke-width="5"/>'+
        '<circle r="55" fill="none" stroke="#c0392b" stroke-width="2"/>'+
        '<text x="0" y="-4" text-anchor="middle" font-family="serif" font-size="30" fill="#c0392b" font-weight="700">长 夜</text>'+
        '<text x="0" y="28" text-anchor="middle" font-family="monospace" font-size="15" fill="#c0392b">CHANGYE</text>'+
      '</g>'+
    '</g>'+
  "")+'</div>';
  const datebox=layer.querySelector('#datebox'),stamp=layer.querySelector('#stamp');
  return{update(p){
    if(datebox)datebox.setAttribute('opacity',clamp((p-.28)/.14,0,1).toFixed(2));
    if(stamp){const k=clamp((p-.52)/.15,0,1);stamp.setAttribute('opacity',k.toFixed(2));
      stamp.setAttribute('transform','translate(650,556) rotate(-13) scale('+lerp(1.6,1,k).toFixed(3)+')');}
  }};
}};
...
{n:"镜34",frame:"切入 · 特写",act:5,start:462,end:478,scene:"labReport",fx:"none",
  subs:[S(1.5,7,"北蚕（画外音）","起初，我以为只是紫外线，高了一点。","vo"),
        S(8,14.5,"北蚕（画外音）","直到我测了血样——阳光留下的毒，我们叫它『光毒』——超标七百倍。我测了六遍。","vo")]},
{n:"镜35",frame:"中景",act:5,start:478,end:494,scene:"labPhone",fx:"none",
  subs:[S(1,8,"北蚕","陈望秋，我今天一步没出门。可我血里的光毒，是正常晒太阳的六十七倍。",""),
        S(9,13,"北蚕","它还在自己往上涨。它在杀我们。",""),
        S(13.5,15.5,"陈望秋（画外音）","……我现在就去实验室。","vo")]},
{n:"镜36",frame:"快速蒙太奇",act:5,start:494,end:520,scene:"montage2",fx:"none",
  subs:[S(.5,5,"陈望秋（画外音）","臭氧层——天空里挡住阳光毒素的那层盾。","vo"),
        S(5.5,9.5,"陈望秋（画外音）","它没有破，它『反』了。阳光，第一次变成了毒。","vo"),
        S(10,15.5,"陈望秋（画外音）","光毒一进身体，就清空免疫力——你这辈子的抵抗力，全部归零，像个婴儿。","vo"),
        S(16,19.5,"陈望秋（画外音）","然后，一场普通的感冒，就能要命。","vo"),
        S(20,25.5,"陈望秋（画外音，绝望的笑）","全球、所有季节，一起逆转。没有一个自然现象能解释——这不是天灾。","vo")]},
{n:"镜37",frame:"灾难蒙太奇",act:5,start:520,end:550,scene:"disaster",fx:"ashHvy",
  subs:[S(3,9,"","那年夏天，一场户外婚礼。所有人都站在阳光下，笑着。","sfx"),
        S(11,18,"","第二天，整个村子，没有一个人，再醒过来。","sfx")]},
{n:"镜38",frame:"中景",act:5,start:550,end:572,scene:"memo",fx:"none",
  subs:[S(2,9.5,"","一份名册。被红笔圈住的，是『千分之二』——天生对光毒免疫的人。","sfx"),
        S(10.5,19,"北蚕（画外音）","他们被收集、抽血、研究。而普通人晒死后，器官被卖掉——换来的钱，正好买这种血。","vo")]},
{n:"镜39",frame:"特写 · 文件",act:5,start:572,end:590,scene:"patent",fx:"none",
  subs:[S(1.5,8,"北蚕（画外音）","后来，我进了生物安全评估组。在技术档案的最底下，我翻到了这个——","vo"),
        S(9,15,"北蚕（画外音）","这层滤光膜的核心专利，在灾难前整整两年，就注册好了。","vo"),
        S(15.5,17.8,"北蚕（画外音）","注册它的，是一家从没人听过的公司：长夜。","vo")]},

/* ===== Act 6 — 轮回与谎言 ===== */
{n:"镜40",frame:"全景",act:6,start:590,end:602,scene:"ruinedCity",fx:"ashHvy",
  subs:[S(3,9.5,"北蚕（画外音）","有一天，我在一袋供血里，发现一张字条。","vo")]},
...
{n:"镜42",frame:"特写",act:6,start:618,end:628,scene:"writeYes",fx:"none",
  subs:[S(2,7.5,"北蚕（画外音）","他问：他们，在说谎吗。","vo")]},
...
{n:"镜46",frame:"日落 · 片名",act:6,start:668,end:682,scene:"finalTitle",fx:"ember",grade:{tint:"#2a1208",tintA:.12,warm:"#ffae5a",sat:1.08,con:1.05,bri:1.04,bloomC:"#ffb060",bloomA:.16,vig:.66},subs:[]}
);
